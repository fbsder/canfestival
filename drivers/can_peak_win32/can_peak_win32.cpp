// can_peak_win32 adapter (http://www.gridconnect.com)
...
class can_peak_win32
   {
   public:
      class error
        {
        };
      can_peak_win32(s_BOARD *board);
      ~can_peak_win32();
...
can_peak_win32::can_peak_win32(s_BOARD *board) : m_port(INVALID_HANDLE_VALUE),
      m_read_event(0),
      m_write_event(0),
      m_d(board->d)
   {
...
can_peak_win32::~can_peak_win32()
   {
   close_rs232();
   }

bool can_peak_win32::send(const Message *m)
   {
   if (m_port == INVALID_HANDLE_VALUE)
      return false;

   // build can_peak_win32 command string
   std::string can_cmd;
...
bool can_peak_win32::receive(Message *m)
   {
   if (m_port == INVALID_HANDLE_VALUE)
      return false;
...
bool can_peak_win32::open_rs232(int port, int baud_rate)
   {
   if (m_port != INVALID_HANDLE_VALUE)
      return true;
...
bool can_peak_win32::close_rs232()
   {
   if (m_port != INVALID_HANDLE_VALUE)
      {
      ::PurgeComm(m_port, PURGE_RXABORT | PURGE_RXCLEAR | PURGE_TXABORT | PURGE_TXCLEAR);
...
bool can_peak_win32::get_can_data(const char* can_cmd_buf, long& bufsize, Message* m)
   {
   if (bufsize < 5)
      {
      bufsize = 0;
...
bool can_peak_win32::set_can_data(const Message& m, std::string& can_cmd)
   {
   // build can_peak_win32 command string
...
extern "C"
   ...
   {
   return (UNS8)reinterpret_cast<can_peak_win32*>(inst)->receive(m);
   }

extern "C"
   UNS8 canSend_driver(void* inst, const Message *m)
   {
   return (UNS8)reinterpret_cast<can_peak_win32*>(inst)->send(m);
   }
...
extern "C"
   ...
   try
      {
      return new can_peak_win32(board);
      }
   catch (can_peak_win32::error&)
      {
      return 0;
      }
   }
...
extern "C"
   ...
   {
   delete reinterpret_cast<can_peak_win32*>(inst);
   return 1;
   }
